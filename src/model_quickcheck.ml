...
(*
(* Deterministic - multiple runs have the same action sequence

   if model = uut we should pass

   create and cleanup called once for each test

   prop failure means test failure prop

   success means test success

   We catch exceptions and return Error _

   test preconditions

   model/uut is recreated for each sequence

   model/uut are recreated during shrinking
*)
...
module Append = struct
  ...
  type ret = int List.t [@@deriving sexp_of]

  let name = "append"
  let update_model model arg = arg :: model
  let update_uut uut arg = 
    let uut' = arg :: uut in
    uut', uut'
  let precondition _ _ = true
end

module ListTest = Setup (TestList) (TestList)

let%expect_test "A simple test" =
  let prop = (fun arg ret model ->  in
  let model = TestList.create () in
  let uut = TestList.create () in
  let seed = Test.Config.Seed.Deterministic (Int.to_string n) in
  let config = { Test.default_config with test_count = 1; shrink_count = 2048; seed } in
  let res = ListTest.run_sequence [(1.0, (module Append))] (module Append) prop model uut config in
  TestList.cleanup uut;
;;
...
module UnitTest = Setup (Unit) (Unit)

module Print = struct
  type arg = int [@@deriving quickcheck, sexp_of]
  type ret = unit [@@deriving sexp_of]

  let name = "print"
  let update_model () i = Caml.Printf.printf "Update model %d\n" i
  let update_uut () i = Caml.Printf.printf "Update UUT %d\n" i, ()

  let precondition () i =
    Caml.Printf.printf "Precondition %d\n" i;
    true
  ;;
end
